...
- Marketplace2 code now lives in `ingestion/marketplace2/fetch_marketplace2_listing.py` (a shim remains at `ingestion/jobs/fetch_marketplace2_listing.py` for compatibility).
- Dockerfile for the marketplace2 job: `ingestion/marketplace2/Dockerfile` (entrypoint `python -m ingestion.jobs.fetch_marketplace2_listing`).
...
- Infra/Terraform wiring is untouched—point it to the new image (`${REGION}-docker.pkg.dev/${PROJECT_ID}/${REPO}/${SERVICE_NAME}:latest`) when you're ready.
